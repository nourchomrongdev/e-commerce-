...
function ProductArt({ tone, label }: { tone: string; label: string }) {
  return <div className={`relative grid aspect-[1.78/1] overflow-hidden rounded-lg bg-gradient-to-br ${tone} p-3`}>
...
export default function MarketplacePage() {
  return <div className="min-h-screen bg-[#f8faff]">
    ...
    <main className="mx-auto max-w-[1560px] px-4 pb-10 pt-4 sm:px-6 lg:px-8">
      ...
      <section id="best-sellers" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{products.map(([name, type, price, tone, label]) => <Link href={`/marketplace/NourChomrong/${name.toLowerCase().replaceAll(" ", "-")}`} key={name} className="group rounded-xl border border-border bg-white p-3 shadow-sm transition hover:-translate-y-1 hover:border-orange-200 hover:shadow-lg"><ProductArt tone={tone} label={label} /><span className="mt-3 inline-block rounded-full bg-orange-50 px-2 py-0.5 text-[9px] font-semibold text-primary">{type}</span><h2 className="mt-1 truncate text-xs font-bold text-heading">{name}</h2><p className="mt-1 text-[10px] text-muted">by <span className="font-medium text-primary">NourChomrong</span></p><div className="mt-2 flex items-end justify-between"><span><b className="text-sm text-heading">{price}</b><small className="mt-1 block text-[10px] text-amber-500">★ 4.8 <span className="text-muted">(124)</span></small></span><span className="grid h-7 w-7 place-items-center rounded-lg bg-primary text-white"><PublicIcon name="shopping-cart" className="h-3.5 w-3.5" /></span></div></Link>)}</section>
    </main>
    <PublicFooter />
  </div>;
}
