...
export default function StorefrontForm({ mode, storefrontName }: StorefrontFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!slug.trim()) nextErrors.slug = "Store URL can't be empty.";
    ...
    const request = isEdit
      ? fetch(`${apiUrl}/creator/storefronts/${encodeURIComponent(decodedName)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storeName: name, slug, description, isPublished, isFeatured, guestPurchase, logoUrl: croppedLogo }) })
