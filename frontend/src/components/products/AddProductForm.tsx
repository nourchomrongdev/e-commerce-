...
export default function AddProductForm({ productsPath, storefrontName, editProduct }: Props & { editProduct?: EditableProduct }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!releases.some((release) => release.files.length > 0)) {
      setError("Add at least one file to a product version before saving.");
    ...
    try {
      const response = await fetch(editProduct ? `${apiUrl}/creator/storefronts/${encodeURIComponent(storefrontName)}/products/${productKey}` : `${apiUrl}/creator/storefronts/${encodeURIComponent(storefrontName)}/products`, {
        ...
        body: JSON.stringify({
          ...
          versions: releases.map((release) => ({ version: release.version.trim(), releaseNotes: release.releaseNotes.trim(), current: release.current, files: release.files, previews: release.previewAssets.map((preview, index) => ({ ...preview, sortOrder: index })) })),
        }),
      });
