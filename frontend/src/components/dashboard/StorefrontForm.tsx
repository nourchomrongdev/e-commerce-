...
export default function StorefrontForm({ mode, storefrontName }: StorefrontFormProps) {
  ...
  const [deleteStep, setDeleteStep] = useState<"name" | "captcha" | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/creator/storefronts/${encodeURIComponent(decodedName)}`, { headers: getAuthHeaders() })
      .then(async (response) => {
  ...
  const previewLogo = croppedLogo || logoSource;
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const request = isEdit
      ? fetch(`${apiUrl}/creator/storefronts/${encodeURIComponent(decodedName)}`, { method: "PUT", headers: { "Content-Type": "application/json", ...getAuthHeaders() }, body: JSON.stringify({ storeName: name, description, isPublished, isFeatured, guestPurchase, logoUrl: croppedLogo }) })
      : fetch(`${apiUrl}/creator/storefronts`, { method: "POST", headers: { "Content-Type": "application/json", ...getAuthHeaders() }, body: JSON.stringify({ storeName: name, slug, description, isPublished, isFeatured, guestPurchase, logoUrl: croppedLogo }) });
  ...
  const confirmDeleteName = () => {
    if (deleteNameConfirmation.trim() !== name.trim()) return;
    setDeleteError("");
    setDeleteStep("captcha");
  };

  const confirmDeleteStorefront = () => {
    if (!captchaValid) return;
    setIsSubmitting(true);
    fetch(`${apiUrl}/creator/storefronts/${encodeURIComponent(decodedName)}`, { method: "DELETE", headers: getAuthHeaders() })
      .then(async (response) => {
        if (!response.ok) {
          const data = await response.json().catch(() => ({}));
  ...
  return (
    <div className="mx-auto w-full max-w-[1080px]">
      ...
      <form onSubmit={handleSubmit} className="overflow-hidden rounded-xl border border-[#e9edf5] bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        <fieldset disabled={isSubmitting} className="min-w-0 disabled:cursor-wait disabled:opacity-70">
        ...
        <footer className="flex flex-col-reverse gap-3 border-t border-[#edf0f5] p-5 sm:flex-row sm:items-center sm:justify-end sm:p-6">{isEdit && <button type="button" onClick={deleteStorefront} className="mr-auto inline-flex items-center justify-center gap-2 rounded-lg border border-red-300 px-4 py-2.5 text-xs font-semibold text-red-600 transition hover:bg-red-50"><PublicIcon name="delete" className="h-3.5 w-3.5" />Delete Storefront</button>}<Link href="/creator/storefront" className="inline-flex items-center justify-center rounded-lg border border-[#e1e5ee] px-5 py-2.5 text-xs font-semibold text-[#33405d] transition hover:border-primary hover:text-primary">Cancel</Link><button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-5 py-2.5 text-xs font-semibold text-white shadow-[0_8px_16px_rgba(255,103,0,0.18)] transition hover:bg-primary-hover disabled:cursor-wait disabled:opacity-60"><PublicIcon name={isEdit ? "check" : "add"} className="h-3.5 w-3.5" />{isSubmitting ? "Saving..." : isEdit ? "Update Storefront" : "Create Storefront"}</button></footer>
        </fieldset>
      </form>
      <Modal
        open={deleteStep !== null}
        title={deleteStep === "name" ? "Confirm storefront deletion" : "Complete CAPTCHA"}
        onClose={() => setDeleteStep(null)}
        footer={(
          <>
            <Button type="button" variant="secondary" size="sm" onClick={() => setDeleteStep(null)}>
              Cancel
            </Button>
            {deleteStep === "name" ? (
              <Button type="button" size="sm" onClick={confirmDeleteName} disabled={isSubmitting || deleteNameConfirmation.trim() !== name.trim()}>
                Continue
              </Button>
            ) : (
              <Button type="button" variant="danger" size="sm" onClick={confirmDeleteStorefront} disabled={isSubmitting || !captchaValid}>
                Delete storefront
              </Button>
            )}
          </>
        ...
        {deleteStep === "name" ? (
          <div>
            <p className="text-sm leading-6 text-muted">
              This will permanently delete <span className="font-semibold text-ink">{name || "this storefront"}</span> and its settings. Type the storefront name to continue.
            </p>
          ...
          <div>
            <p className="text-sm leading-6 text-muted">
              Enter the alphanumeric CAPTCHA to permanently delete <span className="font-semibold text-ink">{name}</span>.
            </p>
            <ClientCaptchaChallenge onValidChange={setCaptchaValid} />
          </div>
        )}
        {deleteError && <p role="alert" className="mt-3 text-xs font-medium text-red-600">{deleteError}</p>}
      </Modal>
