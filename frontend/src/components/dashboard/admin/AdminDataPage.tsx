...
export default function AdminDataPage({ section }: { section: AdminSection }) {
  ...
  return <div className="mx-auto w-full max-w-[1400px]">
    ...
    {config.metrics.length > 0 && <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{config.metrics.map(([label, value, change]) => <article key={label} className="rounded-xl border border-border bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><p className="text-[10px] font-semibold text-muted-soft">{label}</p><span className="grid h-8 w-8 place-items-center rounded-lg bg-orange-50 text-primary"><PublicIcon name={metricIcons[label] ?? "dashboard"} className="h-4 w-4" /></span></div><p className="mt-2 text-xl font-bold text-heading">{value}</p><p className={`mt-1 text-[9px] font-semibold ${change.startsWith("-") ? "text-status-danger" : "text-status-success"}`}>{change}</p></article>)}</div>}
