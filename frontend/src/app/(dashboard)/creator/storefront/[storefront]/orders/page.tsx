...
import { Badge, Button, InputText, MetricTile, Table, TableHeader, TableRow } from "@/components/ui";
...
function SummaryCard({ label, value, detail, icon, tone }: { label: string; value: string; detail: string; icon: string; tone: string }) {
  return <MetricTile label={label} value={value} change={detail} icon={<PublicIcon name={icon as any} className="h-5 w-5" />} iconClassName={tone} />;
}
...
export default function CreatorOrdersPage() {
  ...
  return <div className="mx-auto w-full max-w-[1400px]">
    ...
    <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5"><SummaryCard label="Total Orders" value="248" detail="↑ 18.4% vs. last month" icon="shopping-cart" tone="text-primary" /><SummaryCard label="Total Revenue" value="$1,284.50" detail="↑ 16.7% vs. last month" icon="dollar" tone="text-primary" /><SummaryCard label="Completed Orders" value="215" detail="86.7% of total" icon="verified" tone="text-primary" /><SummaryCard label="Pending Orders" value="18" detail="7.3% of total" icon="notification" tone="text-primary" /><SummaryCard label="Cancelled Orders" value="15" detail="6.0% of total" icon="delete" tone="text-primary" /></section>
