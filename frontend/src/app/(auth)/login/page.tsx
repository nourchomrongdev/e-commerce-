...
function LoginPageContent() {
  ...
  const submitDisabled = loading || !form.email.trim() || !form.password.trim() || Object.values(fieldErrors).some(Boolean);
  ...
  return (
      <>
        ...
        <PreventSubmit onSubmit={submit} disabled={loading}>
          ...
          <div><AuthField name="password" value={form.password} onChange={update("password")} label="Password" type="password" placeholder="Enter your password" error={fieldErrors.password} /><div className="mt-2 flex items-center justify-between text-[10px] text-muted"><label className="flex items-center gap-1.5"><input type="checkbox" className="accent-primary" /> Remember me</label><AuthLink href={routes.auth.forgotPassword()}>Forgot password?</AuthLink></div></div>
          ...
          <a href={`${apiUrl}/auth/paypal`} className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-[#1d4ed8] text-[10px] font-medium text-[#1d4ed8] no-underline transition hover:bg-[#eff6ff]"><img src="/icons/paypal.svg" alt="" aria-hidden="true" className="h-6 w-6" />Continue with PayPal</a>
